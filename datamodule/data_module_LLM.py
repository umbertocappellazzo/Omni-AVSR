...
# We don't need the lengths as CTC is dropped. If we want to test with CTC we can re-integrate them. 
def collate_LLM(batch, tokenizer, modality, is_trainval= True):
    ...
    #elif prompt_type == "base_prompt": # In inference we need to remove the eos token --> [:, :-1].
    #    tokens = tokenizer([prompt + text for text in batch["tokens"]], padding= 'longest', return_tensors="pt").input_ids if is_trainval else tokenizer(prompt, return_tensors="pt").input_ids[:,:-1] 
    #    if is_trainval: # Padding tokens and prompt tokens are set to -100.
    #        labels = torch.tensor([IGNORE_INDEX if el == pad_id or index in range(1,prompt_len+1) else el for index,el in enumerate(tokens)], device = tokens.device)
    #    else:
    #        labels = None
        
    #else:
    #    assert prompt_type == "chat_prompt", ("Error in the prompt field. Accepted prompt_type values: None, base_prompt, chat_prompt")
        # TODO: understand whether [INST], [/INST] must be included in the training.
   #     pass
    
    # Not sure if the unsqueeze is necessary.
    batch_out["tokens"] = tokens#.unsqueeze(1)
    
    batch_out["labels"] = torch.stack(labels) if is_trainval else labels
    
    
    # If we use the Whisper encoder, the model extracts mel-spectrograms through the feature extractor and it again applies padding (but the audios are already padded here).
    if modality == "audio" or modality == "audiovisual" or modality == "audiovisual_avhubert":
