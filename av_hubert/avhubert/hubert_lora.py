...
if DBG:
    from av_hubert.avhubert.hubert_pretraining import (
        AVHubertPretrainingConfig,
        AVHubertPretrainingTask,
    )
    from av_hubert.avhubert.resnet import ResEncoder
    logging.basicConfig(
    ...
    from av_hubert.avhubert.utils import compute_mask_indices
    from av_hubert.avhubert.decoder import TransformerDecoder
...
class SubModel(nn.Module):
    def __init__(self, resnet=None, input_dim=None, cfg=None):
        ...
        self.encoder = TransformerEncoder_lora(cfg) if cfg.encoder_layers > 0 else None
...
@register_model("av_hubert_lora", dataclass=AVHubertConfig)
class AVHubertModel_lora(BaseFairseqModel):
    def __init__(
        self,
        cfg: AVHubertConfig,
        ...
        **kwargs
        ...
        self.encoder = TransformerEncoder_lora(cfg)
    ...
    def build_model(cls, cfg: AVHubertConfig, task: AVHubertPretrainingTask):
        ...
        kwargs = {}
        model = AVHubertModel_lora(cfg, task.cfg, task.dictionaries, **kwargs)
        return model
