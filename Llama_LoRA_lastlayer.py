...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 31 15:46:40 2025
...
class LlamaSdpaAttention_lora(LlamaSdpaAttention):
    def __init__(self, config: LlamaConfig, lora_config: LoRA_config, layer_idx: Optional[int] = None):
        ...
        self.is_last_layer = config.num_hidden_layers == (layer_idx + 1)
    ...
    def forward(
        ...
        **kwargs,
        ...
        Q_lora = self.lora_up_Q[modality](self.lora_down_Q[modality](hidden_states)) if self.is_last_layer else self.lora_up_Q(self.lora_down_Q(hidden_states))
        V_lora = self.lora_up_V[modality](self.lora_down_V[modality](hidden_states)) if self.is_last_layer else self.lora_up_V(self.lora_down_V(hidden_states))
...
class LlamaForCausalLM_lora_lastlayer(LlamaForCausalLM):
