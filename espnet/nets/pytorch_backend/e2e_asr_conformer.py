...
class E2E(torch.nn.Module):
    ...
    def scorers(self):
        ...
        #return dict(decoder=self.decoder, ctc=CTCPrefixScorer(self.ctc, self.eos)) if self.ctc_weight else dict(decoder=self.decoder)
    ...
    def forward(self, x, lengths, label):
        ...
        loss = self.ctc_weight * loss_ctc + (1 - self.ctc_weight) * loss_att #if self.ctc_weight else loss_att

        acc = th_accuracy(
            pred_pad.view(-1, self.odim), ys_out_pad, ignore_label=self.ignore_id
        )

        return loss, loss_ctc, loss_att, acc #if self.ctc_weight else loss, loss_att, acc
