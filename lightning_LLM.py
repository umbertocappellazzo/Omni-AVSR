...
from Llama_MoE_adapters import SMoA_config_llama, DMoA_config_llama
...
class ModelModule_LLM(LightningModule):
        ...
        if args.add_PETF_LLM:
            ...
            if args.add_PETF_LLM == 'adapter':
                ...
                if "Qwen" in args.llm_model:
                    ...
                    lora_config_llm = LoRA_config(args.rank, args.alpha, IS_LLAMA3, IS_TINYLLAMA, IS_LLAMA3_2_3B)
    ...
    def training_step(self, batch, batch_idx):
        if self.args.is_MoE and self.args.apply_load_balancing_loss:   
            train_loss, load_balancing_loss = self.model(batch, is_trainval = True)
            train_loss = train_loss + self.args.load_balancing_loss_coeff*load_balancing_loss
        else:
    ...
    def validation_step(self, batch, batch_idx):
        if self.args.is_MoE and self.args.apply_load_balancing_loss:   
            val_loss, val_load_balancing_loss = self.model(batch, is_trainval = True)
            val_loss = val_loss + self.args.load_balancing_loss_coeff*val_load_balancing_loss
        else:
